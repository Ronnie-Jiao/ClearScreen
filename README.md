...
- 无障碍实现参考了李跳跳 APK 的服务声明形态：服务使用 `exported=false`、`settingsActivity`、通用反馈类型和 Android 12+ 的 `isAccessibilityTool` 配置，并在独立的 `:accessibility` 进程中运行；主界面的 React Native/Expo 进程不会被无障碍服务加载。这不会伪造或绕过系统授权。
